    ...
    <div className="space-y-5 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          tag="LTV"
          label="継続売上"
          value={formatYen(kpis.recurringSales)}
        />
        <KpiCard
          icon="％"
          iconBg="bg-rose-500"
        ...
        <KpiCard
          ...
          tag="成約率"
          label="入会率"
          value={`${formatPercent(kpis.conversionRate)} (${formatCount(kpis.conversionCount)})`}
        />
        <KpiCard
          icon="💬"
          iconBg="bg-slate-700"
          tag="Google"
          label="G口コミ(合計)"
          value={formatCount(kpis.googleReviews, "件")}
        />
        <KpiCard
          icon="❤️"
          iconBg="bg-pink-500"
          tag="Hotpepper"
          label="H口コミ(合計)"
          value={formatCount(kpis.hotpepperReviews, "件")}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-4">
        <RankingCard
          ...
          value={(s) => (
            <span className="text-brand-600">{formatManYen(s.sales)}</span>
          )}
        />
        <RankingCard
          title="入会率 TOP10"
